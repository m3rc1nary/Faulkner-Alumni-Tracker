<?php
    ...
    //Depending on type of user we will redirect to various pages		
            if($row[0] == 'Secretary')	 { header( "location:http://localhost/abc.php"); }
            else if($row[0] == 'Department Chair')	 { header( "location:http://localhost/xyz.php"); }
            else if($row[0] == 'Dean')	 { header( "location:http://localhost/xyz.php"); }
            else if($row[0] == 'Admin')	 { header( "location:CreateUser.php"); 	}
